    ...
    <div className="movies_section">
      ...
      >
        {movies.slice(0, 15).map((e, index) => {
          let img_url;
          if (e.poster_path) img_url = API_IMG + e.poster_path;
          else
            img_url =
              "https://th.bing.com/th/id/OIP.qbcqrRWIikjak63VtqI0DwAAAA?pid=ImgDet&rs=1";
          return (
            <div
              key={index}
              className="movies_items"
              onClick={() => history(`/movie/${e.id}`)}
            >
              ...
              <div className="desc">
                ...
                {query === "popular" && (
                  <p className="popularity">
                    Popularity: {e.popularity} million
                  </p>
                )}
                {query === "upcoming" && (
                  <p className="release_date">Release Date: {e.release_date}</p>
                )}
                {query === "top_rated" && (
                  <div>
